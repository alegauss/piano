...
- 📋 **PI72** (deps: —) **Durations are ticks, so nothing says whether a note is a dotted quarter or what fills the gap after it** — A stave is written in figures and rests, and the format offers neither, so every notation reader would invent the same quantiser. → §PI72
- 📋 **PI73** (deps: PI72) **The piece has no written form: the falling roll is the only way to read what comes next** — A player who reads staff notation gets nothing from this app, and no code here engraves a stave. → §PI73
